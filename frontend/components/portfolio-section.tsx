...
export function PortfolioSection({ data = portfolioData, dictionary }: PortfolioSectionProps) {
    ...
    <div className="space-y-6 md:space-y-8">
      ...
      <div className="flex flex-wrap gap-2 md:gap-3">
        {data.categories.map((category) => (
          ...
          >
            {category === 'all' ? dictionary.all : category}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {filteredProjects.map((project, index) => (
          ...
          >
            ...
            <div className="absolute top-3 right-3 md:top-4 md:right-4 px-2.5 md:px-3 py-1 md:py-1.5 bg-background/90 backdrop-blur-sm border border-border rounded-lg text-xs font-medium text-accent">
              {project.category}
            </div>
          </div>
        ))}
